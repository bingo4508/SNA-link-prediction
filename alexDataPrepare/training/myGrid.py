...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			...
			self.gnuplot_pathname = 'C:\Program Files\gnuplot\bin'
		else:
			# example for windows
...
class Worker(Thread):
	...
	def get_cmd(self,c,g):
		...
		cmdline += ' {0} {1} {2}.model '.format\
			(options.pass_through_string,options.dataset_pathname,options.dataset_title)
		return cmdline
...
class LocalWorker(Worker):
	def run_one(self,c,g):
		...
		result.readlines();
		cmdline = '"' + 'mySvmPredict.exe' + '"'
		cmdline += ' {0} {1}.model {1}.predict'.format\
			(options.test_set,options.dataset_title)
		result = Popen(cmdline,shell=True,stdout=PIPE,stderr=PIPE,stdin=PIPE).stdout
		for line in result.readlines():
			if str(line).find('F-score') != -1:
